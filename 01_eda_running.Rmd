...
```{r, echo = FALSE}
n_runs <- nrow(garmin_data)
```

Now, let's take a look at some key running statistics.  In particular, for the `r n_runs` runs I did, let's take a look at how long I was out there, my average pace, and my distance.
...
Next, for good measure, let's take a quick look at the summary statistics for the `r n_runs-4` runs I completed which were less than 2H 30M in duration. 
...
```{r, echo = FALSE}
pearson <- round(cor(garmin_data$time_numeric, garmin_data$distance), digits = 2)
```

As I did with the tables earlier, let's take a look at the distance against the time I spent running. First, let's examine the correlation.  For this, we'll use the time_numeric variable from the garmin_data data.frame, which represents the time, in seconds, I spent running during each run.  The Pearson's R is `r pearson`---indicating a very strong and positive relationship between the time I spent running and the distance I ran.  That's a relief!  That said, from the previous analyses, we know there were some blips...
